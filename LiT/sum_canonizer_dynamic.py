...
class SumReplacer(ast.NodeTransformer):
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(node.op, ast.Add):
            ...
            new_node = ast.Call(
                func=ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr='sum_lit_layer', ctx=ast.Load()),
...
def sum_canonizer(model, namespace, ignore_layers=[], save=False):
     ...
     for _, module in model.named_modules():
        ...
        if hasattr(module, "sum_lit_layer"):
            raise AttributeError("Please rename your module attribute 'sum_lit_layer' to something else.")
        else:
