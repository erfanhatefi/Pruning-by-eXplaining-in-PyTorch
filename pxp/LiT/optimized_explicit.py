...
class SoftmaxEpsilon(Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        relevance = inputs * (grad_outputs[0] - output * grad_outputs[0].sum(-1, keepdim=True))
...
class MatrixMultiplicationEpsilon(Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        out_relevance = grad_outputs[0]
        # out_relevace = out_relevace / (outputs + MatrixMultiplicationEpsilon.epsilon) * 2

        out_relevance = out_relevance / stabilize(outputs * 2) 

        relevance_a = input_a * torch.matmul(out_relevance, input_b.permute(0, 1, -1, -2))
        relevance_b = input_b * torch.matmul(input_a.permute(0, 1, -1, -2), out_relevance)

        #relevance_a = torch.einsum('bhji,bhip,bhjp->bhji', input_a, input_b, out_relevance)
        #relevance_b = torch.einsum('bhji,bhip,bhjp->bhip', input_a, input_b, out_relevance)
...
class MatrixMultiplicationEpsilon2(Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        #out_relevance.mul_((outputs + LinearEpsilon.epsilon).mul_(2))
        out_relevance = out_relevance / stabilize(outputs * 2) 
...
class SumEpsilon(Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        out_relevance_norm = grad_outputs[0] / stabilize(sum(args))
        
        relevances = tuple(a * out_relevance_norm for a in args)
