...
from LiT.layers import SumLiT, MultiheadAttentionSeparatedLiT, LayerNormTU
from LiT.canonizers import CanonizerReplaceAttribute, CanonizerReplaceModule
...
class VitTorchvisionSumCanonizer(CanonizerReplaceAttribute):
    ...
    def forward_block(self, input: torch.Tensor):
        ...
        x = self.dropout(x)
        # x = x + input
        x = self.sum_layer(x, input)

        y = self.ln_2(x)
        y = self.mlp(y)
        # return x + y
        return self.sum_layer(x, y)
...
class ReplaceAttentionNorm(CanonizerReplaceModule):
    ...
    def create_attention_layer(self, source):
        ...
        dest_layer = self.attn_type(source.embed_dim, source.num_heads, bias=bias, batch_first=source.batch_first,
                                kdim=source.kdim, vdim=source.vdim)
        ...
        if not source._qkv_same_embed_dim:
            ...
            dest_layer.q_proj.proj_weight.data.copy_(source.in_proj_weight.data[:source.embed_dim])
            dest_layer.k_proj.proj_weight.data.copy_(source.in_proj_weight.data[source.embed_dim:source.embed_dim*2])
            dest_layer.v_proj.proj_weight.data.copy_(source.in_proj_weight.data[source.embed_dim*2:source.embed_dim*3])

        dest_layer.out_proj.proj_weight.data.copy_(source.out_proj.weight.data)

        if source.in_proj_bias is not None:
            dest_layer.q_proj.proj_bias.data.copy_(source.in_proj_bias.data[:source.embed_dim])
            dest_layer.k_proj.proj_bias.data.copy_(source.in_proj_bias.data[source.embed_dim:source.embed_dim*2])
            dest_layer.v_proj.proj_bias.data.copy_(source.in_proj_bias.data[source.embed_dim*2:source.embed_dim*3])
